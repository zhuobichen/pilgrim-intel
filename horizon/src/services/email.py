...
class EmailManager:
    ...
    def send_daily_summary(self, summary_md: str, subject: str, subscribers: List[str]):
        ...
        if not self.config.enabled:
            self.console.print("[yellow]⚠️  Email disabled in config, skipping[/yellow]")
            return
        if not subscribers:
            self.console.print("[yellow]⚠️  No subscribers, skipping email[/yellow]")
            return
            ...
            with smtplib.SMTP_SSL(
                ...
                for subscriber in subscribers:
                    ...
                    try:
                        ...
                        self.console.print(f"   ✅ Email sent to {subscriber}")
                    except Exception as e:
                        logger.error(f"Failed to send to {subscriber}: {e}")
                        self.console.print(f"   ❌ Failed to send to {subscriber}: {e}")

                if not success_count:
                    self.console.print(f"[red]❌  Email failed: 0/{len(subscribers)} sent[/red]")
